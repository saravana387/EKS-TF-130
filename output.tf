...
# Outputs the addons installed on the EKS cluster
output "eks_addons" {
  description = "Details of the addons installed on the EKS cluster"
  value = {
    for addon in aws_eks_addon.new_eks_addon :
    addon.addon_name => {
      version          = addon.addon_version,
      arn              = addon.arn,
      service_role_arn = addon.service_account_role_arn
    }
  }
}
